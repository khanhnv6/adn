...
static const CAmount COINMIXING_COLLATERAL = (10 * COIN);
static const CAmount COINMIXING_POOL_MAX = (99999.99 * COIN);

extern CCoinMixingPool coinMixingPool;
extern CCoinMixingSigner coinMixingSigner;
...
{
public:
    /// Is the inputs associated with this public key? (and there is 1000 ADN - checking if valid masternode)
...
private:
    ...
    std::vector<CCoinMixingEntry> entries; // Masternode/clients entries
...
public:
    ...
    void SendCoinMixingDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    ...
    /// Get the denominations for a specific amount of adn.
