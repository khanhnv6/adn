...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "adn.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "adnd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 23202, 23152));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ADN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ADN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via LightningSend) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via LightningSend) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, adn, (coinmixing, lightingsend, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ADN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ADN specific functionality (Masternodes, CoinMixing, LightningSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:23202"));
    ...
    strUsage += HelpMessageOpt("-anonymizeadnamount=<n>", strprintf(_("Keep N ADN anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("LightningSend options:"));
    strUsage += HelpMessageOpt("-enablelightingsend=<n>", strprintf(_("Enable lightingsend, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-lightingsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nLightningSendDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 23203, 23153));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The ADN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ADN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablelightingsend", fEnableLightningSend)) {
        if (SoftSetArg("-lightingsenddepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enablelightingsend=false -> setting -nLightningSendDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ADN Core is shutting down."));
...
#endif
    // Make sure only a single ADN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ADN Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ADN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ADN Core to complete") << "\n";
    ...
    if (fMasterNode) {
        ...
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!coinMixingSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
    ...
    if (nLiquidityProvider != 0) {
        coinMixingPool.SetMinBlockSpacing(std::min(nLiquidityProvider, 100) * 15);
        fEnableCoinMixing = true;
        nCoinMixingRounds = 99999;
    }

    nAnonymizeAdnAmount = GetArg("-anonymizeadnamount", 0);
    if (nAnonymizeAdnAmount > 999999) nAnonymizeAdnAmount = 999999;
    if (nAnonymizeAdnAmount < 2) nAnonymizeAdnAmount = 2;

    fEnableLightningSend = GetBoolArg("-enablelightingsend", fEnableLightningSend);
    nLightningSendDepth = GetArg("-lightingsenddepth", nLightningSendDepth);
    nLightningSendDepth = std::min(std::max(nLightningSendDepth, 0), 60);
    ...
    LogPrintf("nLightningSendDepth %d\n", nLightningSendDepth);
    LogPrintf("CoinMixing rounds %d\n", nCoinMixingRounds);
    LogPrintf("Anonymize ADN Amount %d\n", nAnonymizeAdnAmount);
    ...
    /* Denominations
       ...
       For example:
       1ADN+1000 == (.1ADN+100)*10
       10ADN+10000 == (1ADN+1000)*10
    */
    coinMixingDenominations.push_back((10000 * COIN) + 10000000);
    coinMixingDenominations.push_back((1000 * COIN) + 1000000);
    coinMixingDenominations.push_back((100 * COIN) + 100000);
    coinMixingDenominations.push_back((10 * COIN) + 10000);
    coinMixingDenominations.push_back((1 * COIN) + 1000);
    coinMixingDenominations.push_back((.1 * COIN) + 100);
    /* Disabled till we need them
    coinMixingDenominations.push_back( (.01      * COIN)+10 );
    coinMixingDenominations.push_back( (.001     * COIN)+1 );
    */

    coinMixingPool.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadCheckCoinMixingPool));
