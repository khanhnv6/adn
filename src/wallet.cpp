...
{
    BOOST_FOREACH (CAmount d, coinMixingDenominations)
        if (nInputAmount == d)
            return true;
    return false;
}
...
bool less_then_denom(const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH (CAmount d, coinMixingDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 ADN
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, coinMixingDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 ADN over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputCoinMixingRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nCoinMixingRoundsMin, int nCoinMixingRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 ADN+1 ( bit on if present )
            // bit 1 - 1000 ADN+1
            // bit 2 - 100 ADN+1
            // bit 3 - 10 ADN+1
            // bit 4 - 1 ADN+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 ADN.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough CoinMixing non-denominated funds for this transaction that are not equal 10000 ADN.");
                    } else {
                    ...
                    if (useIX) {
                        strFailReason += " " + _("LightningSend requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-adn-address
                    CScript scriptChange;
...
string CWallet::PrepareCoinMixingDenominate(int minRounds, int maxRounds)
{
    ...
    if (coinMixingPool.GetState() != POOL_STATUS_ERROR && coinMixingPool.GetState() != POOL_STATUS_SUCCESS)
        if (coinMixingPool.GetEntriesCount() > 0)
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(coinMixingPool.sessionDenom, 0.1 * COIN, COINMIXING_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, coinMixingDenominations) {
            ...
            bool fAccepted = false;
            if ((coinMixingPool.sessionDenom & (1 << 0)) && v == ((10000 * COIN) + 10000000)) {
                fAccepted = true;
            } else if ((coinMixingPool.sessionDenom & (1 << 1)) && v == ((1000 * COIN) + 1000000)) {
                fAccepted = true;
            } else if ((coinMixingPool.sessionDenom & (1 << 2)) && v == ((100 * COIN) + 100000)) {
                fAccepted = true;
            } else if ((coinMixingPool.sessionDenom & (1 << 3)) && v == ((10 * COIN) + 10000)) {
                fAccepted = true;
            } else if ((coinMixingPool.sessionDenom & (1 << 4)) && v == ((1 * COIN) + 1000)) {
                fAccepted = true;
            } else if ((coinMixingPool.sessionDenom & (1 << 5)) && v == ((.1 * COIN) + 100)) {
                fAccepted = true;
            }
            if (!fAccepted) continue;
    ...
    if (coinMixingPool.GetDenominations(vOut) != coinMixingPool.sessionDenom) {
    ...
    coinMixingPool.SendCoinMixingDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
int CMerkleTx::GetDepthInMainChain(const CBlockIndex*& pindexRet, bool enableIX) const
{
    ...
    if (enableIX) {
        if (nResult < 6) {
            ...
            if (signatures >= LIGHTINGSEND_SIGNATURES_REQUIRED) {
                return nLightningSendDepth + nResult;
            }
        }
    }
...
int CMerkleTx::GetTransactionLockSignatures() const
{
    ...
    if (!IsSporkActive(SPORK_2_LIGHTINGSEND)) return -3;
    if (!fEnableLightningSend) return -1;
...
{
    if (!fEnableLightningSend) return 0;
