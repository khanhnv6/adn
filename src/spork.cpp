...
bool IsSporkActive(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_2_LIGHTINGSEND) r = SPORK_2_LIGHTINGSEND_DEFAULT;
        if (nSporkID == SPORK_3_LIGHTINGSEND_BLOCK_FILTERING) r = SPORK_3_LIGHTINGSEND_BLOCK_FILTERING_DEFAULT;
...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    if (!coinMixingSigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage)) {
        return false;
    }
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    if (!coinMixingSigner.SetKey(strMasterPrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CMasternodePayments::Sign - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!coinMixingSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CMasternodePayments::Sign - Sign message failed");
        return false;
    }

    if (!coinMixingSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
...
{
    if (strName == "SPORK_2_LIGHTINGSEND") return SPORK_2_LIGHTINGSEND;
    if (strName == "SPORK_3_LIGHTINGSEND_BLOCK_FILTERING") return SPORK_3_LIGHTINGSEND_BLOCK_FILTERING;
...
{
    if (id == SPORK_2_LIGHTINGSEND) return "SPORK_2_LIGHTINGSEND";
    if (id == SPORK_3_LIGHTINGSEND_BLOCK_FILTERING) return "SPORK_3_LIGHTINGSEND_BLOCK_FILTERING";
