...
Value coinmixing(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "coinmixing <adnaddress> <amount>\n"
            "adnaddress, reset, or auto (AutoDenominate)"
    ...
    if (params[0].get_str() == "auto") {
        if (fMasterNode)
            return "CoinMixing is not supported from masternodes";

        return "DoAutomaticDenominating " + (coinMixingPool.DoAutomaticDenominating() ? "successful" : ("failed: " + coinMixingPool.GetStatus()));
    }
    ...
    if (params.size() != 2)
        throw runtime_error(
            "coinmixing <adnaddress> <amount>\n"
            "adnaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) ADN address of current masternode\n"
    ...
    obj.push_back(Pair("state", coinMixingPool.GetState()));
    obj.push_back(Pair("entries", coinMixingPool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted", coinMixingPool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in adn.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode ADN address\n"
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:23202\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:23202\""));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) ADN address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) ADN MN Address\n"
