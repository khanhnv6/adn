...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ADN)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            connect(timer, SIGNAL(timeout()), this, SLOT(coinMixingStatus()));
            timer->start(1000);
        }
    }
...
{
    if (!fLiteMode && !fMasterNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(coinMixingStatus()));
    delete ui;
}
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ADN")
    updateDisplayUnit();
}
...
void OverviewPage::updateCoinMixingProgress()
{
    ...
    QString strAnonymizeAdnAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAdnAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAdnAmount = strAnonymizeAdnAmount.remove(strAnonymizeAdnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAdnAmount + " / " + tr("%n Rounds", "", nCoinMixingRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAdnAmount * COIN) nMaxToAnonymize = nAnonymizeAdnAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAdnAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeAdnAmount));
        strAnonymizeAdnAmount = strAnonymizeAdnAmount.remove(strAnonymizeAdnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAdnAmount + " / " + tr("%n Rounds", "", nCoinMixingRounds);
    } else {
...
void OverviewPage::coinMixingStatus()
{
    ...
    if (((nBestHeight - coinMixingPool.cachedNumBlocks) / (GetTimeMillis() - nLastDSProgressBlockTime + 1) > 1)) return;
    nLastDSProgressBlockTime = GetTimeMillis();

    if (!fEnableCoinMixing) {
        if (nBestHeight != coinMixingPool.cachedNumBlocks) {
            coinMixingPool.cachedNumBlocks = nBestHeight;
            updateCoinMixingProgress();
    ...
    QString strStatus = QString(coinMixingPool.GetStatus().c_str());
    ...
    if (coinMixingPool.sessionDenom == 0) {
        ui->labelSubmittedDenom->setText(tr("N/A"));
    } else {
        std::string out;
        coinMixingPool.GetDenominationsToString(coinMixingPool.sessionDenom, out);
    ...
    if (!fEnableCoinMixing) {
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked) {
            ...
            if (!ctx.isValid()) {
                ...
                coinMixingPool.cachedNumBlocks = std::numeric_limits<int>::max();
                QMessageBox::warning(this, tr("CoinMixing"),
