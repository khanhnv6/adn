...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ADN);
    ...
    if (!settings.contains("nAnonymizeAdnAmount"))
        settings.setValue("nAnonymizeAdnAmount", 1000);

    nCoinMixingRounds = settings.value("nCoinMixingRounds").toLongLong();
    nAnonymizeAdnAmount = settings.value("nAnonymizeAdnAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAdnAmount"))
        SoftSetArg("-anonymizeadnamount", settings.value("nAnonymizeAdnAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in adn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAdnAmount:
            return QVariant(nAnonymizeAdnAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case CoinMixingRounds:
            ...
            break;
        case AnonymizeAdnAmount:
            nAnonymizeAdnAmount = value.toInt();
            settings.setValue("nAnonymizeAdnAmount", nAnonymizeAdnAmount);
            emit anonymizeAdnAmountChanged(nAnonymizeAdnAmount);
            break;
        case CoinControlFeatures:
