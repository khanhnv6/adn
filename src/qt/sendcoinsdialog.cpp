                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    // ADN specific
    QSettings settings;
    if (!settings.contains("bUseCoinMixing"))
        settings.setValue("bUseCoinMixing", false);
    if (!settings.contains("bUseLightningSend"))
        settings.setValue("bUseLightningSend", false);

    bool useCoinMixing = settings.value("bUseCoinMixing").toBool();
    bool useLightningSend = settings.value("bUseLightningSend").toBool();
    if (fLiteMode) {
        ui->checkUseCoinMixing->setChecked(false);
        ui->checkUseCoinMixing->setVisible(false);
        ui->checkLightningSend->setVisible(false);
        CoinControlDialog::coinControl->useCoinMixing = false;
        CoinControlDialog::coinControl->useLightningSend = false;
    } else {
        ui->checkUseCoinMixing->setChecked(useCoinMixing);
        ui->checkLightningSend->setChecked(useLightningSend);
        CoinControlDialog::coinControl->useCoinMixing = useCoinMixing;
        CoinControlDialog::coinControl->useLightningSend = useLightningSend;
    }

    connect(ui->checkUseCoinMixing, SIGNAL(stateChanged(int)), this, SLOT(updateDisplayUnit()));
    connect(ui->checkLightningSend, SIGNAL(stateChanged(int)), this, SLOT(updateLightningSend()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkLightningSend->isChecked()) {
        recipients[0].useLightningSend = true;
        strFunds += " ";
        strFunds += tr("and LightningSend");
    } else {
        recipients[0].useLightningSend = false;
    }
...
void SendCoinsDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (model && model->getOptionsModel()) {
        uint64_t bal = 0;
        QSettings settings;
        settings.setValue("bUseCoinMixing", ui->checkUseCoinMixing->isChecked());
        if (ui->checkUseCoinMixing->isChecked()) {
...
void SendCoinsDialog::updateDisplayUnit()
{
    ...
    setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(), model->getAnonymizedBalance(),
        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
    CoinControlDialog::coinControl->useCoinMixing = ui->checkUseCoinMixing->isChecked();
    coinControlUpdateLabels();
...
void SendCoinsDialog::updateLightningSend()
{
    QSettings settings;
    settings.setValue("bUseLightningSend", ui->checkLightningSend->isChecked());
    CoinControlDialog::coinControl->useLightningSend = ui->checkLightningSend->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid ADN address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUseCoinMixing->setChecked(CoinControlDialog::coinControl->useCoinMixing);
